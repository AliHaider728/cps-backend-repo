...
async function fetchModelRows(modelName: string): Promise<any[]> {
...
async function fetchModelRecord(modelName: string, id: string): Promise<any | null> {
  const result = await query(
...
async function persistModelRecord(modelName: string, record: any): Promise<any> {
...
async function deleteModelRecord(modelName: string, id: string): Promise<void> {
...
class BaseDocument {
  ...
  constructor(model: any, data: any, options: { select?: string } = {}) {
    this.__model = model;
...
class QueryBuilder {
  ...
  constructor(model: any, resolver: () => Promise<any>, { single = false } = {}) {
    this.model = model;
  ...
  async exec(): Promise<any> {
    ...
    const mapped = next.map(
      (item) =>
        new this.model.DocumentClass(
          this.model,
          applySelect(item, this.selectSpec, this.model.hiddenFields),
          {
...
export function createModel(config: ModelConfig): any {
