...
export const PRACTICE_NAME_SQL: string = `NULLIF(TRIM(COALESCE(
  p.name,
  pr.data->>'name',
...
export const CLINICAL_SYSTEM_SQL: string = `NULLIF(TRIM(COALESCE(
  s.clinical_system,
  rs.clinical_system,
  p.clinical_system,
...
export const PRACTICE_JOIN_SQL: string = `
...
export async function loadPracticeMeta(practiceId: string | null | undefined): Promise<PracticeMeta | null> {
  ...
  if (!row) return { id: pid, name: null, contractType: "PCN", clinical_system: null };

  const data: Record<string, any> = { ...(row.client_data || {}), ...(row.practice_data || {}) };
...
export function serviceCodeForPracticeType(contractOrType: string | null = "PCN"): string {
  const t = String(contractOrType || "PCN").toUpperCase();
  if (t === "EA" || t.includes("ENHANCED")) return "EA";
  if (t === "GP" || t === "DIRECT") return "GP";
  return "PCN";
}

export async function applyCoverShiftDefaults(payload: Record<string, any> = {}, practiceId?: string): Promise<Record<string, any>> {
  const isCover =
...
export function stripUuidDisplayName(name: string | null | undefined): string | null {
  if (!name) return null;
