...
function matchesCondition(actual, expected) {
  ...
  if (expected && typeof expected === "object" && !Array.isArray(expected)) {
    ...
    if ("$in" in expected) {
      ...
      return haystack.some((item) => String(item) === String(actual));
    }
  }
...
function normalizeStoredRecord(row, fixedData = {}) {
  if (!row) return null;
...
async function fetchTableRows(tableModel) {
...
async function fetchRepositoryRows(repository) {
...
async function fetchRepositoryRecord(repository, id) {
  const result = await query(
...
async function persistRepositoryRecord(repository, record) {
...
async function deleteRepositoryRecord(repository, id) {
...
class QueryBuilder {
  constructor(repository, resolver, { single = false } = {}) {
...
export function createRepository(config) {
  ...
  class Repository {
    ...
    static async findByIdAndDelete(id) {
      const existing = await fetchRepositoryRecord(this, normalizeId(id));
