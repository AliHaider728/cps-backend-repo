...
export const SQL_PRACTICE_NAME: string = `NULLIF(TRIM(COALESCE(
  p.name,
  pr.data->>'name',
...
export const SQL_CLINICAL_SYSTEM = (shiftAlias: string = "s"): string =>
...
export const SQL_PRACTICE_JOINS = (practiceIdCol: string = "s.practice_id"): string => `
...
export const SQL_ROTA_PRACTICE_JOINS = (surgeryIdCol: string = "rs.surgery_id"): string => `
...
export const SQL_WORKING_SHIFT_FILTER_SHIFTS: string = `AND s.status NOT IN ('annual_leave', 'sick', 'cppe', 'gap')`;

export const SQL_WORKING_SHIFT_FILTER_ROTA: string = `AND rs.shift_type NOT IN ('annual_leave', 'sick', 'cppe_training', 'bank_holiday', 'gap')`;
