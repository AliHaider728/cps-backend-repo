...
export const updateOnboarding = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const before: any = await Clinician.findById(id).lean();
    ...
    const updated: any = await Clinician.findByIdAndUpdate(
      id,
      { onboarding: merged },
      { new: true }
    );
...
/* ─── SEND WELCOME PACK ──────────────────────────────────── */
export const sendWelcomePack = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const id = toId(req.params.id);
    if (!id) return res.status(400).json({ message: "Invalid clinician id" });
    ...
    const nextOnboarding = {
      ...
      welcomePackSentBy: (req as any).user?._id || null,
    };
