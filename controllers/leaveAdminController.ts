...
const enrichEntries = async (entries: any[]) => {
  const clinicianIds = [...new Set(entries.map((e) => String(e.clinician)))];
  const clinicians = clinicianIds.length
    ? await Clinician.find({}).lean().then((all: any) =>
        all.filter((c: any) => clinicianIds.includes(String(c._id)))
      )
    : [];
  const clinMap = new Map(clinicians.map((c: any) => [String(c._id), c]));
  ...
  return entries.map((e) => {
    const c: any = clinMap.get(String(e.clinician));
    const approver: any = userMap.get(String(e.approvedBy));
    return {
      ...e,
...
export const listAdminLeaves = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const status = String((req.query.status as string) || "pending").toLowerCase();
    let entries: any[] = await ClinicianLeaveEntry.find({}).lean();
    ...
    entries.sort((a, b) => {
      const da: any = new Date(a.createdAt || a.startDate || 0);
      const db: any = new Date(b.createdAt || b.startDate || 0);
...
export const reviewLeave = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const before: any = await ClinicianLeaveEntry.findById(leaveId).lean();
    ...
    if (action === "reject") {
      ...
      await logAudit(req as any, "REJECT_LEAVE", "ClinicianLeaveEntry", {
        // @ts-ignore
        resourceId: leaveId,
    ...
    const updated: any = await ClinicianLeaveEntry.findByIdAndUpdate(
      leaveId,
      {
        approved: true,
        rejected: false,
    ...
    await logAudit(req as any, "APPROVE_LEAVE", "ClinicianLeaveEntry", {
      // @ts-ignore
      resourceId: leaveId,
...
export const getLeaveReport = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    for (const c of clinicians as any[]) {
      ...
      const balances: any[] = calcAllBalances(entries);

      for (const b of balances) {
        const contract = b.contract;
        const annual = entries.filter(
          (e: any) => e.contract === contract && e.leaveType === "annual"
        );
        const pending = annual
          .filter((e: any) => e.approved !== true && e.rejected !== true)
          .reduce((s: number, e: any) => s + Number(e.days || 0), 0);
        const approved = annual
          .filter((e: any) => e.approved === true)
          .reduce((s: number, e: any) => s + Number(e.days || 0), 0);
