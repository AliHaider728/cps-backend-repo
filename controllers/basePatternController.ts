...
export const createBasePattern = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const days = Array.isArray(req.body.day_of_week) ? req.body.day_of_week : [req.body.day_of_week];
  const created = [];
  for (const day of days) {
    created.push(await BasePattern.create({ ...req.body, day_of_week: Number(day), created_by: (req as any).user?._id || (req as any).user?.id }));
  }
  return ok(res, created, "Base pattern created", 201);
});

export const getClinicianBasePatterns = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const patterns = await BasePattern.findByClinician(req.params.clinician_id);
  return ok(res, patterns);
});

export const updateBasePattern = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  const pattern = await BasePattern.update(req.params.id, req.body || {});
  return ok(res, pattern, "Base pattern updated");
});

export const deactivateBasePattern = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
