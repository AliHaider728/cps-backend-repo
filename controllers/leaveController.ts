/**
 * controllers/leaveController.ts — Module 3
 *
...
const eachDateISO = (startDate: any, endDate: any) => {
  const out = [];
...
const inDateRange = (iso: any, from: any, to: any) => {
...
export const syncLeaveToRota = async ({ clinicianId, leaveType, startDate, endDate, actorId, sourceLeaveId }: any) => {
  const rotaStatus =
  ...
  if (leaveType === "annual") {
    ...
    const supervisionDates = new Set((supervision || []).map((s: any) => String(s.sessionDate || "").slice(0, 10)));
    const clashes = dates.filter((d) => supervisionDates.has(d));
    if (clashes.length > 0) {
      // Trainer alert + audit log marker
      await logAudit({ user: { _id: actorId, role: "system" } } as any, "LEAVE_SUPERVISION_CLASH_ALERT", "ClinicianLeaveEntry", {
  ...
  if (leaveType === "sick") {
    ...
    const impactedMeetings = (meetings || []).filter((m: any) =>
      inDateRange(m.sessionDate, from, to)
    );

    const stakeholderIds = new Set<string>();
    impactedMeetings.forEach((m: any) => {
    ...
    roleStakeholders
      .filter((u: any) => ["ops_manager", "training", "super_admin"].includes(String(u.role || "")))
      .forEach((u: any) => stakeholderIds.add(String(u._id)));

    const stakeholders = roleStakeholders.filter((u: any) => stakeholderIds.has(String(u._id)));
    ...
    await logAudit({ user: { _id: actorId, role: "system" } } as any, "ABSENCE_NOTIFICATION_TRIGGERED", "ClinicianLeaveEntry", {
      ...
      after: {
        ...
        startDate,
        endDate,
        notifiedUsers: stakeholders.map((u: any) => ({ id: u._id, role: u.role, email: u.email })),
        impactedMeetings: impactedMeetings.map((m: any) => ({
          id: m._id,
...
const loadLeavePayload = async (id: any) => {
  const clinician = await Clinician.findById(id).lean();
  if (!clinician) return null;
  const entries = await ClinicianLeaveEntry.find({ clinician: id }).lean();
  entries.sort((a: any, b: any) => new Date(b.startDate || 0).getTime() - new Date(a.startDate || 0).getTime());
  return {
    entries,
...
const hasApprovedLeaveClash = async (clinicianId: any, startDate: any, endDate: any) => {
...
export const getMyLeave = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const id = await resolveClinicianIdForUser((req as any).user);
    if (!id) {
...
/* ─── ADD ────────────────────────────────────────────────── */
export const addLeave = async (req: Request, res: Response, next: NextFunction) => {
  try {
    // @ts-ignore
    const id = await assertClinicianAccess(req, req.params.id);
    ...
    const adminApproved =
      body.approved === true ||
      body.approved === "true" ||
      ["super_admin", "director", "ops_manager"].includes(reqUser?.role);
    ...
    const entry = await ClinicianLeaveEntry.create({
      ...
      approved,
      rejected: false,
      approvedBy: approved ? (reqUser?._id || null) : null,
...
export const updateLeave = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (leaveType === "annual" && (body.days != null || body.startDate || body.endDate)) {
      const all = await ClinicianLeaveEntry.find({ clinician: id }).lean();
      const others = all.filter((e: any) => String(e._id) !== String(entryId));
    ...
    const reqUser = (req as any).user;
    if (isApproving) {
      body.approvedBy = reqUser?._id || null;
