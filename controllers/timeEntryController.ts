...
export const getActiveEntry = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const clockIn = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = String((req as any).user._id || (req as any).user.id || "");
    const clinicianId = (req as any).user.clinicianId || null;
    ...
    await logAudit(req as any, "CLOCK_IN", "TimeEntry", {
      resourceId: entry.id,
...
export const clockOut = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await logAudit(req as any, "CLOCK_OUT", "TimeEntry", {
...
export const getAdminSummary = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const clinicianSummaries = clinicians.rows.map((row: any) => {
      const data = row.data || {};
    ...
    const totalShifts = clinicianSummaries.reduce((sum: number, clinician: any) => sum + clinician.totalShiftsMonth, 0);
    const totalPlanned = clinicianSummaries.reduce((sum: number, clinician: any) => sum + clinician.plannedHoursMonth, 0);
    const totalActual = clinicianSummaries.reduce((sum: number, clinician: any) => sum + clinician.actualHoursMonth, 0);
    const totalClockedIn = clinicianSummaries.filter((clinician: any) => clinician.currentlyClockedIn).length;
...
export const getClinicianTimeEntriesAdmin = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const entries = entriesResult.rows.map((row: any) => ({ id: row.id, ...row.data }));
