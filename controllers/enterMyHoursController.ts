...
const calcWorkedHours = (startTime: any, endTime: any, breakMinutes: any = 0) => {
...
const monthYearFromDate = (dateWorked: any) => {
  const d = new Date(`${String(dateWorked).slice(0, 10)}T00:00:00`);
  if (Number.isNaN(d.getTime())) return { month: null, year: null };
  return { month: d.getMonth() + 1, year: d.getFullYear() };
};

export async function getMyEnterHours(req: Request, res: Response, next: NextFunction) {
  try {
    const clinicianId = await resolveClinicianIdForUser((req as any).user);
    if (!clinicianId) {
    ...
    if (month && year) entries = entries.filter((e: any) => Number(e.month) === month && Number(e.year) === year);
    entries.sort((a: any, b: any) => String(b.dateWorked || "").localeCompare(String(a.dateWorked || "")));
...
export async function upsertMyEnterHours(req: Request, res: Response, next: NextFunction) {
  try {
    const clinicianId = await resolveClinicianIdForUser((req as any).user);
    if (!clinicianId) {
      return res.status(404).json({ message: "No clinician profile linked to your account." });
    ...
    let entry = null;
    if (entryId) {
      const existing: any = await EnterMyHoursEntry.findById(entryId).lean();
      ...
      const existingRows: any = await EnterMyHoursEntry.find({
        clinician: clinicianId,
        shiftId,
        dateWorked: String(dateWorked).slice(0, 10),
      }).lean();
...
export async function submitMyEnterHours(req: Request, res: Response, next: NextFunction) {
  try {
    const clinicianId = await resolveClinicianIdForUser((req as any).user);
    if (!clinicianId) {
      return res.status(404).json({ message: "No clinician profile linked to your account." });
...
export async function listManagerEnterHours(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (month && year) entries = entries.filter((e: any) => Number(e.month) === month && Number(e.year) === year);
    if (status) entries = entries.filter((e: any) => String(e.managerApprovalStatus) === status);
    entries.sort((a: any, b: any) => String(b.dateWorked || "").localeCompare(String(a.dateWorked || "")));
...
export async function reviewManagerEnterHours(req: Request, res: Response, next: NextFunction) {
  try {
    const role = (req as any).user?.role;
    if (!MANAGER_ROLES.includes(role)) {
      return res.status(403).json({ message: "Access denied: insufficient permissions" });
    ...
    const entry: any = await EnterMyHoursEntry.findById(req.params.id).lean();
