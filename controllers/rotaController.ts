...
const ok = (res: any, data: any, message = "OK", status = 200) =>
...
export async function checkRestrictedClinician(clinicianId: any, practiceId: any) {
...
export async function checkMandatoryCompliance(clinicianId: any) {
  const id = toMongoId(clinicianId);
  if (!id) {
    const err: any = new Error("Invalid clinician id");
    err.statusCode = 400;
    throw err;
  }

  const docs = await ClinicianComplianceDoc.find({ clinician: id }).lean();
  const approved = docs.filter((d: any) => d.status === "approved" && !isExpired(d));

  const missing: string[] = [];
  for (const req of REQUIRED_COMPLIANCE) {
    const found = approved.some((d: any) => {
...
export function validateCoverEntry(entry: any = {}) {
  const isCover = entry.is_cover === true || entry.is_cover === "true";

  if (isCover) {
    if (String(entry.project_code || "").trim() !== "COVER") {
      const err: any = new Error("Cover shifts must use project_code = COVER");
      err.statusCode = 400;
      throw err;
    }
    const service = String(entry.service_code || "").toUpperCase();
    if (!["PCN", "GP", "EA"].includes(service)) {
      const err: any = new Error("Cover shifts must use service_code PCN | GP | EA");
      err.statusCode = 400;
      throw err;
    }
  }
  ...
  if (!allowed.includes(status)) {
    const err: any = new Error(`Invalid shift status: "${status}"`);
    err.statusCode = 400;
    throw err;
  }

  if (status === "cover" && !isCover) {
    const err: any = new Error("status=cover requires is_cover=true");
    err.statusCode = 400;
    throw err;
  }

  if (status === "gap" && entry.clinician_id) {
    const err: any = new Error("Gap shifts must not have a clinician_id");
    err.statusCode = 400;
    throw err;
  }
}

export const checkRestrictedClinicianEntry = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const checkMandatoryComplianceEntry = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
function buildRotaEmailHTML(rotaData: any[], month: any, year: any) {
...
const monthRange = (month: any, year: any) => {
...
const calcRotaEntryHours = (shiftStart: any, shiftEnd: any) => {
...
export const getRotaGrid = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const allClinicians = Array.from(clinicianMap.values()).sort((a, b: any) =>
    ...
    const cliniciansWithShifts = Array.from(byClinician.values()).filter(
      (row: any) => Object.keys(row.shifts).length > 0
    );
...
export const getClinicianRota = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const generateMonthlyRota = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const createShift = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const payload: any = {
      ...
      created_by:     toUUID((req as any).user?._id || (req as any).user?.id) || toMongoId((req as any).user?._id || (req as any).user?.id) || null,
    };
    ...
    if (payload.clinician_id) {
      ...
      if (!compliance.passed) {
        const canOverride = ["super_admin", "ops_manager"].includes(String((req as any).user?.role || ""));
        if (!canOverride) {
...
export const getGapReport = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const gapsWithUrgency = gaps.map((g: any) => {
    ...
    return ok(res, {
      ...
      urgent: gapsWithUrgency.filter((g: any) => g.urgent).length,
    });
  } catch (err: any) {
    next(err);
  }
...
/* ─── POST /api/rota/cover ──────────────────────────────────────────────── */
export const assignCover = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const coverPayload: any = {
      ...
      source:    "manual",
      created_by: toUUID((req as any).user?._id || (req as any).user?.id) || null,
    };
...
export const getCoverRequests = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const sendRotaToClient = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const distribution = await RotaDistribution.create({
      ...
      month,
      year,
      sent_by:          toUUID((req as any).user?._id || (req as any).user?.id) || null,
    ...
    await ContactHistory.create({
      ...
      createdBy:  toUUID((req as any).user?._id || (req as any).user?.id) || null,
      metadata: {
        timestamp:       new Date().toISOString(),
        sent_by:         (req as any).user?._id || (req as any).user?.id,
        recipient_emails: recipients,
      },
    });
...
export const seedShiftsFromJson = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const emailToId   = new Map(
      clinicians
        ...
        .map((c: any) => [String(c.email).toLowerCase(), String(c._id || c.id)])
    );
...
export const getRotaById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const shiftId = toUUID(req.params.id);
    if (!shiftId) return fail(res, 400, "Invalid id");
    const shift: any = await Shift.findById(shiftId);
...
/* ─── Aliases ────────────────────────────────────────────────────────────── */
const currentClinicianId = (req: any) =>
  normalizeClinicianId((req as any).user?.clinicianId) ||
  normalizeClinicianId((req as any).user?.clinician_id);

async function resolveRequestClinicianId(req: any) {
  let id = currentClinicianId(req);
  if (!id && (req as any).user?.role === "clinician") {
    id = await resolveClinicianIdForUser((req as any).user);
    if (id) (req as any).user.clinicianId = id;
  }
  return id;
}

const sqlMonthRange = (month: any, year: any) => {
  const range = monthRange(month, year);
...
const mapShiftTableRow = (row: any = {}) => {
...
const shiftDedupeKey = (shift: any) =>
  [
...
const mergeShiftLists = (primary: any[], secondary: any[]) => {
...
const fetchRotaShiftsFromRotaTable = async ({ month, year, clinicianId, range }: any) => {
...
const fetchRotaShiftsFromShiftsTable = async ({ clinicianId, range }: any) => {
...
const fetchRotaShifts = async ({ month, year, clinicianId = null }: any) => {
...
const fetchAllRotaShiftsFromRotaTable = async (clinicianId: any) => {
  const params: any[] = [];
  let clinicianFilter = "";
  if (clinicianId) {
    params.push(clinicianId);
...
const fetchAllRotaShiftsFromShiftsTable = async (clinicianId: any) => {
  const params: any[] = [];
  let clinicianFilter = "";
  if (clinicianId) {
    params.push(clinicianId);
  ...
  const result = await query(
    `SELECT s.*,
            COALESCE(c.full_name, cr.data->>'fullName', cr.data->>'name') AS clinician_name,
            NULLIF(TRIM(COALESCE(p.name, pr.data->>'name', pr.data->>'practiceName', cl.data->>'name')), '') AS practice_name,
            NULLIF(TRIM(COALESCE(s.clinical_system, pr.data->>'clinicalSystem', pr.data->>'clinicalSystem')), '') AS clinical_system,
            pc.name AS pcn_name
       FROM shifts s
       LEFT JOIN clinicians c ON c.id::text = s.clinician_id::text
       LEFT JOIN app_records cr ON cr.model = 'Clinician' AND cr.id = s.clinician_id::text
...
const fetchAllRotaShiftsForClinician = async (clinicianId: any) => {
...
async function mirrorShiftToRotaShifts(shift: any) {
...
const getTimesheetEntries = async (timesheetId: any) => {
  const result = await query(
    `SELECT te.*,
...
const refreshTimesheetTotal = async (timesheetId: any) => {
  const result = await query(
    `UPDATE timesheets
...
const ensureTimesheetEntries = async (timesheet: any, month: any, year: any) => {
  const range = sqlMonthRange(month, year);
  if (!range) return;

  const clinicianId = normalizeClinicianId(timesheet.clinician_id) as string;
  const shifts = await fetchRotaShifts({ month, year, clinicianId });
  const working = shifts.filter((s: any) => {
...
export const getMonthlyRota = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const month = parseIntStrict(req.query.month);
    const year = parseIntStrict(req.query.year);
    if (!monthRange(month, year)) return fail(res, 400, "month and year are required");
    const clinicianId =
      (req as any).user?.role === "clinician" ? await resolveRequestClinicianId(req) : null;
    const shifts = await fetchRotaShifts({ month, year, clinicianId });
...
export const generateMonthlyRotaFromPatterns = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    let created = 0;
    let skipped = 0;
    const userId = toUUID((req as any).user?._id || (req as any).user?.id);
...
export const getRotaGaps = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await query(
      `SELECT rs.*,
    ...
    return ok(res, { gaps, total: gaps.length, urgent: gaps.filter((g: any) => g.urgency === "urgent").length, critical: gaps.filter((g: any) => g.urgency === "critical").length });
  } catch (err: any) {
    next(err);
  }
};

export const sendRotaToClients = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
const isAllScope = (req: any) =>
  req.query.scope === "all" ||
  req.query.all === "true" ||
  req.query.all === "1";

export const getMyRota = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getTimesheetForMonth = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const clinicianId = await resolveRequestClinicianId(req);
    if (!clinicianId) return fail(res, 403, "Clinician profile is not linked to this user");
...
export const upsertTimesheetEntryForShift = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const clinicianId = await resolveRequestClinicianId(req);
    if (!clinicianId) return fail(res, 403, "Clinician profile is not linked to this user");
...
export const updateTimesheetEntry = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const clinicianId = await resolveRequestClinicianId(req);
...
export const submitTimesheet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const incomplete = entries.filter((entry: any) => !entry.start_time || !entry.end_time || (entry.is_cover && (entry.project_code !== "COVER" || !entry.service_code)));
    // @ts-ignore
...
export const getPendingTimesheets = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await query(
      `SELECT ts.*,
...
export const getTimesheetDetail = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const timesheetId = toUUID(req.params.id);
    const result = await query(
...
export const getClinicianTimesheetForAdmin = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const approveTimesheet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await query(
      ...
      [toUUID(req.params.id), toUUID((req as any).user?._id || (req as any).user?.id)]
    );
    if (!result.rows[0]) return fail(res, 404, "Timesheet not found");
...
export const rejectTimesheet = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const result = await query(
      `UPDATE timesheets
        ...
        WHERE id = $1 RETURNING *`,
      [toUUID(req.params.id), toUUID((req as any).user?._id || (req as any).user?.id), reason]
    );
    if (!result.rows[0]) return fail(res, 404, "Timesheet not found");
