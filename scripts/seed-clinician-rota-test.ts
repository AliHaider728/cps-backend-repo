...
async function upsertAppRecord(model: any, id: any, data: any) {
  await query(
...
async function insertShift({ clinicianId, practiceId, date, status = "working" }: any) {
  const id = uuidv4();
  const hours = 8;
  await query(
    `INSERT INTO shifts (
...
function weekdaysInMonth(year: any, month: any) {
  const dates = [];
